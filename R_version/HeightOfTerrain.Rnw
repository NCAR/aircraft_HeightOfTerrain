...
TdbData <- "/scr/raf_data/TerrainDataR"
...
Rscript HeightOfTerrain.R $project $flight $Dir lt_s lt_n lg_w lg_e Tdb")
   quit()
}
   ...
   # find the database file that contains this:
         ...
         # if it's already present, skip download
           if (!file.exists(zipFileName)) {
             url <- sprintf("https://www.viewfinderpanoramas.org/dem3/%s", zipFileName)
...
for (i in 1:n) {
  ...
  url <- sprintf ("https://137.227.248.58/pqs.php?x=%f&y=%f&units=Meters&output=json", lgg[i], ltt[i])
...
GGALT <- ncvar_get(netCDFfile, "GGALT")
Data <- data.frame(LATC = as.vector(LATC), LONC = as.vector(LONC), GGALT = as.vector(GGALT))
...
varSFC <- ncvar_def ("SFC_SRTM", "m", netCDFfile$dim["Time"], -32767., "Elevation of the Earth's surface below the aircraft position, WGS-84") 
varALTG <- ncvar_def ("ALTG_SRTM", "m", netCDFfile$dim["Time"], -32767., "Altitude of the aircraft above the Earth's surface, WGS-84")
newfile <- ncvar_add (netCDFfile, varSFC, verbose=TRUE)
